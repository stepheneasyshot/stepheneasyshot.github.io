...
tcpdump -i eth0• 捕获 100 个经过 eth0 接口的 HTTP 数据包（端口 80）：
tcpdump -i eth0 -c 100 port 80• 捕获进出主机 192.168.1.100 的所有 ICMP 数据包，并保存到文件 icmp.pcap 中：
...
用户被限制安装应用
在开发者选项里将「USB安装」打开，如果已经打开了，那先关闭再打开。
...
通过 Gradle 的 assembleDebug 或 assembleRelease 重新编译，或者 Generate Signed APK
...
输出应用安装路径
输出示例:
adb shell pm path ecarx.weatherpackage:/data/app/ecarx.weather-1.apk
四、与应用交互
