...
![jvm_ram](/assets/img/blog/blogs_jvm_ram_simple.png){:width ="400" height="200" loading="lazy"}
...
1. 有一个JVM参数 `-XX:PretenureSizeThreshold` ，默认值是 `0` ，表示任何情况都先把对象分配给 `Eden` 区。若设置为1048576字节，也就是1M。则表示当创建的对象大于1M时，就会直接把这个对象放入到老年区，就根本不会经过新生区了。这么做的原因：大对象在经历复制算法进行GC的时候会降低性能。
1. 如果新生区中的某个对象 **经历了15次GC** 后，还是没有被回收掉，那么它就会被转入老年区。
1. 如果当 `Survivor1` 区不足以存放 `Eden` 区和 `Survivor0` 的存活对象时，就将存活对象 **直接放到年老代** 。
