...
运行时可以通过两种方式找到您的原生方法。您可以 请使用 RegisterNatives 注册它们；也可以让运行时动态查询它们 和dlsym。**RegisterNatives 的优势在于，您可以提前 还可以检查这些符号是否存在 导出除 JNI_OnLoad 之外的任何内容。这样做的好处是让运行时 因为它需要编写的代码略少一些。**
