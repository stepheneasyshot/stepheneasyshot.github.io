...
由于它**满足 ? extends TextView 的限制条件**，所以 get 出来的对象，肯定是 TextView 的子类型，根据多态的特性，能够赋值给 TextView，啰嗦一句，赋值给 View 也是没问题的。
...
## `reified` 关键字
最后介绍一下reifeid关键字，`reified` 关键字用于 **内联函数 (inline functions)** 的泛型类型参数。它解决了 Java/Kotlin 泛型类型擦除的问题，允许你在运行时访问泛型类型信息。

由于类型擦除，你不能直接在运行时检查一个泛型类型：
