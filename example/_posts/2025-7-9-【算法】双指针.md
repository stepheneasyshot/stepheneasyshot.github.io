...
### 思路1 双指针
根据图示，可以比较容易想到使用双指针，因为要做区间限制需要有两个下标。双指针共同往中间移动，直到相遇，指针移动时，记录下所组合到的最大面积。问题点就是改移动哪一个指针。

比较通俗的想法是，移动短的那个指针，因为移动长的那个指针，容器的高度不会增加，而宽度会减少，所以容器的面积一定是减少的。数学证明也是如此。
...
### 思路1 排序加双指针
题目要求找元素组合，并未对下标做限制，所以可以先排序，然后使用双指针。
