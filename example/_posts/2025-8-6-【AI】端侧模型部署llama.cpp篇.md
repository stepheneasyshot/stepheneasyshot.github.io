...
如果输入是 4 个词，每个词用 1024 维的向量表示，则 X 的形状是 **(4, 1024)**。

#### B. 权重张量 (Weight Tensors)
模型需要学习三组权重矩阵来将输入 X 转化为查询（Query）、键（Key）和值（Value）向量。
...
* **过程：** 在 GPU/TPU 上，张量计算框架（如 PyTorch/TensorFlow）会并行执行：**(序列长度 4 ) X (嵌入维度 1024)** 的输入，与嵌入维度 **1024 X 1024** 的权重矩阵相乘。
* **结果 Q 的形状：** **(4, 1024)**。
...
同理，K 和 V 也通过张量乘法并行计算出来：

$$\mathbf{K} = \mathbf{X} \cdot \mathbf{W}_K$$

$$\mathbf{V} = \mathbf{X} \cdot \mathbf{W}_V$$

结果 K 和 V 的形状也都是 **(4, 1024)**。

### 3. 核心计算：注意力分数的计算
同样代表输入序列中的每一个词，但它包含的是这个词的真正语义信息。

**目标：** 计算 Q 和 K 之间的相关性分数。

$$\text{Attention}(Q, K, V) = \text{Softmax}(\frac{QK^T}{\sqrt{d_k}})V$$

除数d_k 是 Key 向量的维度缩放，这一步是为了防止点积结果过大，导致 `Softmax` 函数进入梯度消失的区域。

一次张量乘法 K 需要先转置 T。形状为 (4, 1024) 的 Q 张量与形状为 (1024, 4) 的 K^T 张量相乘。结果分数的形状：(4, 4).
...
| 优势 | 描述 |
| :--- | :--- |
| **模型尺寸减小** | 将 FP32（4 字节）量化到 INT4（0.5 字节），理论上模型大小可以**缩小8倍**。这使得模型可以部署到内存有限的设备上。 |
